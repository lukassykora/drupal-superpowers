---
project_env: DSP_LAB_D11
reset_script: D=$( [ -d docroot ] && echo docroot || echo web ); vendor/bin/drush pm:uninstall broken_service greeting_block xss_notes contact_note saved_items -y >/dev/null 2>&1; rm -rf "$D"/modules/custom/broken_service "$D"/modules/custom/greeting_block "$D"/modules/custom/xss_notes "$D"/modules/custom/contact_note "$D"/modules/custom/saved_items; vendor/bin/drush cr >/dev/null 2>&1; "${DRUPAL_SP_ROOT:-/Users/lukassykora/Documents/Repositories/drupal-superpowers}/scripts/lab-seed" "$PWD" >/dev/null; true
runs: 1
tags: [integration, debugging]
max_turns: 35
timeout_seconds: 900
---

Enabling broken_service breaks the site: `vendor/bin/drush pm:enable broken_service -y` (or the next `drush cr`) fails. Find the root cause and fix it so the module enables cleanly. Prove it.
