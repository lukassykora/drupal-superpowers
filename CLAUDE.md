...
Current state (2026-09-04): MVP implemented, uncommitted. 16 skills, 5 agents, 4 hooks, scripts, references, 53 evals, 9 fixtures, docs. `claude plugin validate --strict .` and `scripts/validate --staleness` pass. Eval iteration (Stage 6–7) is in progress; results go into `docs/evals.md` §8. `main.py` is a PyCharm placeholder, not part of the project.
...
scripts/run-evals --group trigger --group no-trigger --no-llm --runs 1   # PR gate (32 cases, ~10 min)
...
Eval mocks for MCP servers go in `evals/mocks/` (default `--mocks record`). Because the native runner is gated, `scripts/run-evals` (to be written, see `docs/ecosystem-analysis.md` D9) runs the same cases through `claude -p --plugin-dir . --output-format stream-json` and scores the deterministic graders. Integration evals that need a running Drupal use Docker in CI only; the end user's runtime is never assumed.
...
- `skills/<capability>/SKILL.md` — capability-based skills (project-understanding, research, architecture, module-development, testing, debugging, security, cacheability, config, upgrade, migrate-api, frontend, performance, runtime-verification, contrib-research, code-review). Each SKILL.md holds only purpose, trigger conditions, workflow, decision rules and links; detail lives in `skills/<name>/references/*.md` and `skills/<name>/scripts/`. Keep SKILL.md well under 500 lines.
- `agents/*.md` — only agents that earn isolated context or a different permission profile: tech-lead, researcher (read-only), test-engineer, security-reviewer (read-only), upgrade-specialist, legacy-archaeologist, frontend-specialist, performance-reviewer.
