...
4. **Docker Compose**: the first of `docker-compose.yml|yaml`, `compose.yml|yaml` with a service named like `php|web|app|drupal|appserver|cli` (else the first service) and the `docker` binary → `docker compose exec <svc> …`.
...
- Docker Compose recipe for the lab: `fixtures/lab-compose/` (added with the integration evals).
