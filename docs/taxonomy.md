...
| 5 | JSON helper for scripts | `php` → `python3` → `node`, grep fallback for the core version only. | A Drupal host always has PHP; macOS/Linux CI has python3; node is common. Never `jq` (not guaranteed). |
...
- **User-invocable aliases**: `understand-project`, `debug`, `audit`, `review`, `verify`, `upgrade`, `setup-mcp` are the `user-invocable: true` skills; there is no separate command layer.
- **One description budget**: ≤ 300 characters, third person, starts with "Use when", trigger conditions and symptoms only, no workflow verbs (then, run, dispatch, review as a step), no "MUST".
