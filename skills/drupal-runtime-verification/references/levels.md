...
| database updates | `drush updatedb:status` | pending updates listed (there is no dry-run for `updb`; `-n` auto-confirms) |
| config | `drush config:status`, `drush cim --diff --no` (Drush 12+) | drift is a finding, not something to auto-fix |
...
|---|---|---|
| HTTP | `curl -sSi <site_url>/path` (anonymous), with a session cookie or `drush user:login --uri=<site_url> --name=<user>` link for authenticated | status code, headers (`X-Drupal-Cache`, `X-Drupal-Dynamic-Cache`, `Cache-Control`), body excerpt |
