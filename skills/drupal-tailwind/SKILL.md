...
4. **Wire the build into Drupal** ([references/setup.md](references/setup.md)): compiled CSS goes to a file the theme owns (`css/style.css`), is declared in `THEME.libraries.yml`, and is attached globally via `libraries:` in `THEME.info.yml` or per-component. A library pointing at the **source** CSS is a defect to fix, not to report: the source still contains `@import "tailwindcss" …`, which Drupal's aggregator does not resolve (see below). Repoint it at the compiled file even when you cannot run the build here, and say that the file needs rebuilding.
...
6. **Verify** — on top of `drupal-frontend`'s gates: every `*.libraries.yml` entry resolves to compiled output, never to the source entry file (check this even when you changed nothing else); the build ran and the output changed (`VERIFY L1 build`, command plus output size or hash); the classes you rely on are in the compiled CSS (`grep -c 'badge-primary' css/style.css`), which is the only honest check that the scan surface is right; `drush cr` and a page load with aggregation **on**; admin routes and the CKEditor form still render.

## Drupal-specific traps

Verified and sourced in [references/drupal-integration.md](references/drupal-integration.md):
...
- **Aggregation keeps modern CSS but mishandles a layered `@import`**: not inlined, yet hoisted to the top of the aggregate where its path breaks. Compiled output has no `@import` left.
...
- New theme → Tailwind 4, CSS-first, `source(none)` plus explicit `@source`; CLI or Vite depending on what the project already runs.
- Existing v3 theme → migrate as its own task with the class inventory in hand; `safelist`, `corePlugins` and `separator` have no v4 equivalent.
- Front-end theme also serves admin routes → drop Preflight or scope it; do not patch the admin UI afterwards with overrides.
- A utility is missing → find why the scanner missed it. `!important` or a hand-written rule hides a scan-surface bug that will hit the next class too.
- Component styling → utilities in the component's Twig; if it needs a real class, `@utility` (v4) or `@layer components`, not scattered `@apply`.
...
| Thought | Reality |
|---|---|
| "The class is in the Twig, it will be generated" | Only if that file is in the scan surface *and* the class is a literal. `'badge-' ~ variant` is not. |
| "I'll just add `@source '**/*'`" | That scans `node_modules` and every contrib module. List the directories. |
| "Preflight is fine, the admin has its own theme" | Only if an admin theme is configured for admin routes; check `/admin/appearance` and `use_admin_theme`. |
| "Point the library at `src/input.css`, Drupal will handle it" | It does not resolve `@import "tailwindcss"`; the page ships a broken import and no utilities. |
| "The utility exists in the browser, so the build is fine" | Watch-mode output differs from a production build. Verify against the compiled file. |
| "Tailwind 4 is a drop-in upgrade" | `safelist`, `corePlugins` and `separator` were removed; a theme relying on a safelist breaks silently. |
