...
4. **Drift and import** ([references/drift.md](references/drift.md)): `drush config:status`, `drush cim --preview=diff`; read the diff; decide per item: export (site is right), import (repo is right), split (environment-specific), or fix code. `cim -y` only after the diff is understood and on LOCAL/DISPOSABLE, or with explicit approval elsewhere (the guard blocks it otherwise).
...
7. Record `VERIFY` lines: schema validated (kernel test or `config:inspect`), `config:status` clean or explained, import preview reviewed.
...
- Multilingual sites: config translation lives in `language/<langcode>/` inside the sync directory.
