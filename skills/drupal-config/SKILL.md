...
4. **Drift and import** ([references/drift.md](references/drift.md)): `drush config:status`, `drush cim --diff --no` (Drush 12+; `--no` aborts at the prompt); read the diff; decide per item: export (site is right), import (repo is right), split (environment-specific), or fix code. `cim -y` only after the diff is understood and on LOCAL/DISPOSABLE, or with explicit approval elsewhere (the guard blocks it otherwise).
