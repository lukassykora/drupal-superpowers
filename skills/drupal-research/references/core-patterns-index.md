...
|---|---|---|
| Content entity type with bundles, revisions, translations | `modules/node/src/Entity/Node.php`, `modules/node/src/NodeAccessControlHandler.php` | attribute `#[ContentEntityType]` from 11.1; annotation `@ContentEntityType` in all 10.x and 11.0 |
...
| Controller with DI | `modules/node/src/Controller/NodeController.php` | `ContainerInjectionInterface::create()`; services `autowire: true` since 9.3; controllers via `AutowireTrait` since 10.2 |
...
| Queue worker | `lib/Drupal/Core/Queue/QueueWorkerInterface.php`, `modules/locale/src/Plugin/QueueWorker/LocaleTranslation.php` | `#[QueueWorker]` attribute 10.3+ |
| Cron hook | 11.x: `modules/system/src/Hook/SystemHooks.php` (`#[Hook('cron')]`), `modules/file/src/Hook/CronHook.php`; 10.x: `modules/node/node.module` `node_cron()` | `node_cron()` no longer exists in 11.x |
...
| Functional test for a route + permission | `modules/node/tests/src/Functional/NodeAccessPagerTest.php`, `modules/node/tests/src/Functional/NodeAccessBaseTableTest.php`, `modules/system/tests/src/Functional/` | `drupalLogin`, `assertSession()->statusCodeEquals(403)` |
...
| State vs config vs tempstore | `modules/system/src/Form/CronForm.php` (state), `lib/Drupal/Core/TempStore/PrivateTempStoreFactory.php` (`tempstore.private`, used in `modules/views_ui`) | |
