...
7. **Verify against the target**: phpcs, phpstan (deprecation rules on), PHPUnit, then install/enable on the target. When the project has no target environment, build one per core with `"${CLAUDE_PLUGIN_ROOT}/scripts/drupal-lab" matrix …` ([references/compatibility-matrix.md](references/compatibility-matrix.md)) and fill the per-core table; then `drush updb`, `drush cr`, the key user workflows, logs. Record `VERIFY` lines per level; unverified steps are `NOT VERIFIED`.
...
- Multi-version support (`^10.3 || ^11`) only when the compatibility matrix is green on both cores (`drupal-lab matrix`); otherwise narrow the constraint.
...
- Procedural hooks stay procedural in an upgrade unless the task is the OOP conversion; `#[Hook]` needs ≥ 11.1 (`drupal-facts check oop-hooks <version>`).
